/**
  This file contains several steps in our project, including brute-force, approximation graph creation with 2-5 dimension, if you want to create higher dimension
  approximation, follow the pattern talked in the report. We can only choose only one methond from Brute-force and approximation for creation a graph. The training 
  and testing data should be under the same folder with this file.
  
  Author: Weidi Zhang
  Debug: Weidi Zhang, Shuyu Lyu, Tianzhe Wang
...
object knnPredict {
  ...
  def main(args: Array[String]) {
    ...
    val a = (0 to trainSize - 1).map(i => {   
      ...
      if (ssss(0) == "List(None") {   // choose input data
        knnVertex(None, arr)
      }
   ...
   val tstSize = tstData.length
    ...
    var errorcounter = 0
    val c = (0 to tstSize - 1).map(i => {   // calculate the error rate
  ...
  def knnGraph(a:Seq[knnVertex], k:Int) = {
    ...
    val e = v.map(v1 => (v1._1, a2.map(v2 => (v2._1, v1._2.dist(v2._2)))
      ...
      .flatMap(x => x._2.map(vid2 =>
        Edge(x._1, vid2,
          1 / (1+a2(vid2.toInt)._2.dist(a2(x._1.toInt)._2)))))  // distance calculation
    Graph(v,e)
  }

  /***     2-dimension  approximation graph creation  */
  ...
  def knnGraphApprox(a:Seq[knnVertex], k:Int) = {
    ...
    val xRange = minMax._2 - minMax._1   // range calculation
    val yRange = minMax._4 - minMax._3

    def calcEdges(offset: Double) =     // partition vertices
      v.map(x => (math.floor((x._2.pos(0) - minMax._1)
        ...
        x))
        .groupByKey(n*n)
        .mapPartitions(ap => {    // do the brute force inside
    ...
    val e = calcEdges(0.0).union(calcEdges(1)) // give the offset and union the result
      .distinct
      .map(x => (x.srcId,x))
      .groupByKey
      .map(x => x._2.toArray
    ...
    val n = 3
    val minMax =
      v.map(x => (x._2.pos(0), x._2.pos(0), x._2.pos(1), x._2.pos(1), x._2.pos(2), x._2.pos(2)))
        .reduce((a,b) => (math.min(a._1,b._1), math.max(a._2,b._2),
          math.min(a._3,b._3), math.max(a._4,b._4),math.min(a._5,b._5), math.max(a._6,b._6)))
    val Range1 = minMax._2 - minMax._1
    val Range2 = minMax._4 - minMax._3
    val Range3 = minMax._5 - minMax._6
    ...
    def calcEdges(offset: Double) =
      v.map(x => (math.floor((x._2.pos(0) - minMax._1)
        ...
        + math.floor((x._2.pos(1) - minMax._3)
        / Range2 * (n-1) + offset)
        + math.floor((x._2.pos(2) - minMax._5)
        / Range3 * (n-1)*n*n + offset),
        x))
        .groupByKey(n*n*n)   // the exponent should increase with the dimension
        .mapPartitions(ap => {
          val af = ap.flatMap(_._2).toList
          af.map(v1 => (v1._1, af.map(v2 => (v2._1, v1._2.dist(v2._2)))
            .toArray
    ...
    val e = calcEdges(0.0).union(calcEdges(0.3))
      .distinct
      .map(x => (x.srcId,x))
      .groupByKey
      .map(x => x._2.toArray
   ...
   /***     4-dimension  
    ...
    val n = 3
    val minMax =
      v.map(x => (x._2.pos(0), x._2.pos(0), x._2.pos(1), x._2.pos(1), x._2.pos(2), x._2.pos(2), x._2.pos(3), x._2.pos(3)))
        .reduce((a,b) => (math.min(a._1,b._1), math.max(a._2,b._2),
          math.min(a._3,b._3), math.max(a._4,b._4),math.min(a._5,b._5), math.max(a._6,b._6),math.min(a._7,b._7), math.max(a._8,b._8)))
    val Range1 = minMax._2 - minMax._1
    val Range2 = minMax._4 - minMax._3
    val Range3 = minMax._5 - minMax._6
    val Range4 = minMax._7 - minMax._8
    ...
    def calcEdges(offset: Double) =
      v.map(x => (math.floor((x._2.pos(0) - minMax._1)
        ...
        + math.floor((x._2.pos(2) - minMax._5)
        / Range3 * (n-1)*n*n + offset)
        + math.floor((x._2.pos(3) - minMax._7)
        / Range3 * (n-1)*n*n*n + offset),
        x))
        .groupByKey(n*n*n*n)   // the exponent should increase with the dimension
        .mapPartitions(ap => {
          val af = ap.flatMap(_._2).toList
          af.map(v1 => (v1._1, af.map(v2 => (v2._1, v1._2.dist(v2._2)))
            .toArray
...
/***     5-dimension  
    ...
    val n = 3
    val minMax =
      v.map(x => (x._2.pos(0), x._2.pos(0), x._2.pos(1), x._2.pos(1), x._2.pos(2), x._2.pos(2), x._2.pos(3), x._2.pos(3), x._2.pos(4), x._2.pos(4)))
        .reduce((a,b) => (math.min(a._1,b._1), math.max(a._2,b._2),
          math.min(a._3,b._3), math.max(a._4,b._4),math.min(a._5,b._5), math.max(a._6,b._6),math.min(a._7,b._7), math.max(a._8,b._8),math.min(a._9,b._9), math.max(a._10,b._10)))
    val Range1 = minMax._2 - minMax._1
    val Range2 = minMax._4 - minMax._3
    val Range3 = minMax._5 - minMax._6
    val Range4 = minMax._7 - minMax._8
    val Range5 = minMax._10 - minMax._9
    ...
    def calcEdges(offset: Double) =
      v.map(x => (math.floor((x._2.pos(0) - minMax._1)
        ...
        + math.floor((x._2.pos(3) - minMax._7)
        / Range3 * (n-1)*n*n*n + offset)
        + math.floor((x._2.pos(4) - minMax._9)
        / Range3 * (n-1)*n*n*n*n + offset),
        x))
        .groupByKey(n*n*n*n*n)   // the exponent should increase with the dimension
        .mapPartitions(ap => {
          val af = ap.flatMap(_._2).toList
          af.map(v1 => (v1._1, af.map(v2 => (v2._1, v1._2.dist(v2._2)))
            .toArray
  ...
  def semiSupervisedLabelPropagation(g:Graph[knnVertex,Double],
                                     maxIterations:Int = 0) = {
    val maxIter = if (maxIterations == 0) g.vertices.count / 2   // stop condition
    else maxIterations
    ...
    do {
        ...
        g2.aggregateMessages[Tuple2[Option[Int],HashMap[Int,Double]]](
          ctx => {
            ctx.sendToSrc((ctx.srcAttr._2.classNum,
              if (ctx.dstAttr._2.classNum.isDefined)   // original labeled vertex: propagating its class
          ...
          (a1, a2) => {
            if (a1._1.isDefined)    
              ...
              (None, a1._2 ++ a2._2.map{  // non labeled vertices: receieve all other info
                case (k,v) => k -> (v + a1._2.getOrElse(k,0.0)) })
          }
        )

      val newVClassVoted = newV.map(x => (x._1,   // merge: get the shortest one
      ...
      isChanged = g2.vertices.join(newVClassVoted)  // see if meet the stop condition
